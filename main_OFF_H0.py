...
def optimizer(W, x, N, M, Ri, batch_thetai, batch_M, batch_N, ri, variance):
    alpha = 1
    gamma = 2
    rho = 0.5
    sigma = 0.5
    ...
    while flag_stop == 1:
        ...
        stacked = tf.stack([v, u], 3)
        W1 = tf.gather_nd(tf.expand_dims(tf.expand_dims(W, axis=2), axis=3),
                          tf.squeeze(stacked, axis=0))  # , batch_dims=-1)
        # crop
        zero = tf.constant(0, dtype=tf.double)
        ...
        W1 = tf.image.crop_to_bounding_box(tf.squeeze(W1, axis=-1), idx_ul1.numpy(), idx_ul2.numpy(),
                                           idx_dr1.numpy() - idx_ul1.numpy(),
                                           idx_dr2.numpy() - idx_ul2.numpy())
        ...
        u, v = polar2cart(s, batch_thetai, batch_M, batch_N)
        stacked = tf.stack([v, u], 3)
        W2 = tf.gather_nd(tf.expand_dims(tf.expand_dims(W, axis=2), axis=3),
                          tf.squeeze(stacked, axis=0))  # , batch_dims=-1)
        # crop
        zero = tf.constant(0, dtype=tf.double)
        ...
        W2 = tf.image.crop_to_bounding_box(tf.squeeze(W2, axis=-1), idx_ul1.numpy(), idx_ul2.numpy(),
                                           idx_dr1.numpy() - idx_ul1.numpy(),
                                           idx_dr2.numpy() - idx_ul2.numpy())
        ...
        u, v = polar2cart(s, batch_thetai, batch_M, batch_N)
        stacked = tf.stack([v, u], 3)
        W3 = tf.gather_nd(tf.expand_dims(tf.expand_dims(W, axis=2), axis=3),
                          tf.squeeze(stacked, axis=0))  # , batch_dims=-1)
        # crop
        zero = tf.constant(0, dtype=tf.double)
        ...
        W3 = tf.image.crop_to_bounding_box(tf.squeeze(W3, axis=-1), idx_ul1.numpy(), idx_ul2.numpy(),
                                           idx_dr1.numpy() - idx_ul1.numpy(),
                                           idx_dr2.numpy() - idx_ul2.numpy())
        ...
        u, v = polar2cart(s, batch_thetai, batch_M, batch_N)
        stacked = tf.stack([v, u], 3)
        Wr = tf.gather_nd(tf.expand_dims(tf.expand_dims(W, axis=2), axis=3), tf.squeeze(stacked, axis=0))
        # crop
        zero = tf.constant(0, dtype=tf.double)
        ...
        Wr = tf.image.crop_to_bounding_box(tf.squeeze(Wr, axis=-1), idx_ul1.numpy(), idx_ul2.numpy(),
                                           idx_dr1.numpy() - idx_ul1.numpy(),
                                           idx_dr2.numpy() - idx_ul2.numpy())
        ...
        if E_sorted[0] <= Er < E_sorted[2]:
            ...
            u, v = polar2cart(s, batch_thetai, batch_M, batch_N)
            stacked = tf.stack([v, u], 3)
            We = tf.gather_nd(tf.expand_dims(tf.expand_dims(W, axis=2), axis=3), tf.squeeze(stacked, axis=0))
            # crop
            zero = tf.constant(0, dtype=tf.double)
            ...
            We = tf.image.crop_to_bounding_box(tf.squeeze(We, axis=-1), idx_ul1.numpy(), idx_ul2.numpy(),
                                               idx_dr1.numpy() - idx_ul1.numpy(),
                                               idx_dr2.numpy() - idx_ul2.numpy())
            ...
            u, v = polar2cart(s, batch_thetai, batch_M, batch_N)
            stacked = tf.stack([v, u], 3)
            Wc = tf.gather_nd(tf.expand_dims(tf.expand_dims(W, axis=2), axis=3), tf.squeeze(stacked, axis=0))
            # crop
            zero = tf.constant(0, dtype=tf.double)
            ...
            Wc = tf.image.crop_to_bounding_box(tf.squeeze(Wc, axis=-1), idx_ul1.numpy(), idx_ul2.numpy(),
                                               idx_dr1.numpy() - idx_ul1.numpy(),
                                               idx_dr2.numpy() - idx_ul2.numpy())
            # linear patterns computer
...
def alpha_validation(W, K, alpha, batch_thetai, batch_M, batch_N, ri, N, M, Ri):
    ...
    noise_rc = tf.gather_nd(tf.expand_dims(tf.expand_dims(W, axis=2), axis=3),
                            tf.squeeze(stacked, axis=0))
    # crop
    zero = tf.constant(0, dtype=tf.double)
    ...
    noise_rc = tf.image.crop_to_bounding_box(tf.squeeze(noise_rc, axis=-1), idx_ul1.numpy(),
                                             idx_ul2.numpy(),
                                             idx_dr1.numpy() - idx_ul1.numpy(),
                                             idx_dr2.numpy() - idx_ul2.numpy())
    #
...
with tf.device('/gpu:5'):
    ...
    images_set = ['../../official_Code/DATASET_RADIAL_CORRECTION/test_parameters/in_camera/NIKON_COOLPIX_S9100/true/misto/*.jpg',
                  '../../official_Code/DATASET_RADIAL_CORRECTION/test_parameters/in_camera/NIKON_COOLPIX_S9100/true/misto/*.jpg',
                  '../../official_Code/DATASET_RADIAL_CORRECTION/test_parameters/in_camera/NIKON_COOLPIX_S9100/true/misto/*.jpg',
                  '../../official_Code/DATASET_RADIAL_CORRECTION/test_parameters/in_camera/NIKON_COOLPIX_S9100/true/misto/*.jpg',
                  '../../official_Code/DATASET_RADIAL_CORRECTION/test_parameters/in_camera/canon_sx210is_sub/true/misto/*.jpg']
    
    outfile_name = 'OUTPUT_H0.mat'
    ...
    for idx_fing in range(len(Fingeprint_list)):
        ...
        for im_name in images:
            ...
            for k in range(k_max):
                if k == 0:
                    ...
                    for a in alphas:
                        ...
                        noise_rc = tf.gather_nd(tf.expand_dims(tf.expand_dims(noise, axis=2), axis=3),
                                                tf.squeeze(stacked, axis=0))
                        # crop
                        zero = tf.constant(0, dtype=tf.double)
                        where = tf.not_equal(noise_rc[0:int(M / 2), :], zero)
            ...
            #validation
            print('VALIDATION')
            validity_array.append(alpha_validation(noise, Fingerprint_tf, alpha_best, batch_thetai, batch_M, batch_N, ri, N, M, Ri))
            time_array[2].append(time.time() - start)

            # Nelder-Mead
            a = optimizer(noise_LP, alpha_best, N, M, Ri, batch_thetai, batch_M, batch_N, ri, variance)
            #final PCE
